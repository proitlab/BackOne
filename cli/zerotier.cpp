/*
 * BackOne - Network Virtualization Everywhere
 * Copyright (C) 2011-2022  BackOne, Inc.  https://backone.cloud/
 *
...
{
	std::cout << "BackOne Newer-Spiffier CLI " << ZEROTIER_ONE_VERSION_MAJOR << "." << ZEROTIER_ONE_VERSION_MINOR << "." << ZEROTIER_ONE_VERSION_REVISION << std::endl;
	std::cout << std::endl;
	std::cout << "Configuration path: " << getSettingsFilePath() << std::endl;
	std::cout << std::endl;
	std::cout << "Usage: backone [-option] [@name] <command> [<command options>]" << std::endl;
	std::cout << std::endl;
	...
	std::cout << "  cli-add-zt @name <url> <auth>       - Add a BackOne service" << std::endl;
	std::cout << "  cli-add-central @name <url> <auth>  - Add BackOne Central instance" << std::endl;
	std::cout << std::endl;
	std::cout << "BackOne Service Commands:" << std::endl;
	...
	std::cout << "  peers                               - List BackOne VL1 peers" << std::endl;
	...
	std::cout << "  id-generate [<vanity prefix>]       - Generate a BackOne identity" << std::endl;
...
static std::tuple<int,std::string> GET(const CLIState &state,const std::map<std::string,std::string> &headers,const std::string &url)
{
	...
	curl_easy_setopt(curl,CURLOPT_USERAGENT,"BackOne-CLI");
...

	{
		...
		if (state.settings.empty()) {
			...
			state.settings = {
				{ "configVersion", 1 },
				{ "things", {
					{ "manage.backone.cloud", {
						{ "type", "central" },
						{ "url", "https://manage.backone.cloud/" },
						{ "auth", "" }
					}},
					{ "local", {
						{ "type", "one" },
				...
				{ "defaultController", "@manage.backone.cloud" },
				{ "defaultOne", "@local" }
			};
			...
			if (OSUtils::readFile((oneHome + ZT_PATH_SEPARATOR_S + "authtoken.secret").c_str(),authToken)&&OSUtils::readFile((oneHome + ZT_PATH_SEPARATOR_S + "backone.port").c_str(),portStr)) {
			...
			if (initSuccess) {
				...
				std::cerr << "INFO: initialized new config at " << getSettingsFilePath() << " but could not auto-init local BackOne service config from " << oneHome << " -- you will need to set local service URL and port manually if you want to control a local instance of BackOne. (This happens if you are not root/administrator.)" << std::endl;
			}
		}
	}
