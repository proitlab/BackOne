...
one:	zeroidc $(CORE_OBJS) $(ONE_OBJS) one.o mac-agent 
	$(CXX) $(CXXFLAGS) -o backone $(CORE_OBJS) $(ONE_OBJS) one.o $(LIBS) rustybits/target/libzeroidc.a
	# $(STRIP) backone
...
#cli:	FORCE
#	$(CXX) $(CXXFLAGS) -o backone cli/zerotier.cpp osdep/OSUtils.cpp node/InetAddress.cpp node/Utils.cpp node/Salsa20.cpp node/Identity.cpp node/SHA512.cpp node/C25519.cpp -lcurl
#	$(STRIP) backone

selftest: $(CORE_OBJS) $(ONE_OBJS) selftest.o
	$(CXX) $(CXXFLAGS) -o backone-selftest selftest.o $(CORE_OBJS) $(ONE_OBJS) $(LIBS) rustybits/target/libzeroidc.a
...
controller-builder: _buildx FORCE
	docker buildx build --platform linux/arm64,linux/amd64 --no-cache -t registry.backone.cloud/backone/ctlbuild:latest -f ext/central-controller-docker/Dockerfile.builder . --push

controller-run: _buildx FORCE
	docker buildx build --platform linux/arm64,linux/amd64 --no-cache -t registry.backone.cloud/backone-central/ctlrun:latest -f ext/central-controller-docker/Dockerfile.run_base . --push

central-controller-docker: _buildx FORCE
	docker buildx build --platform linux/arm64,linux/amd64 --no-cache -t registry.backone.cloud/backone-central/ztcentral-controller:${TIMESTAMP} -f ext/central-controller-docker/Dockerfile --build-arg git_branch=$(shell git name-rev --name-only HEAD) . --push
	@echo Image: registry.backone.cloud/backone-central/ztcentral-controller:${TIMESTAMP}
	
clean:
	rm -rf MacEthernetTapAgent *.dSYM build-* *.a *.pkg *.dmg *.o node/*.o controller/*.o service/*.o osdep/*.o ext/http-parser/*.o $(CORE_OBJS) $(ONE_OBJS) backone backone-idtool backone-selftest backone-cli backone doc/node_modules zt1_update_$(ZT_BUILD_PLATFORM)_$(ZT_BUILD_ARCHITECTURE)_* rustybits/target/
